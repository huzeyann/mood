...
def nystrom_ncut_fixed_sample_wrapper(features, n_eig, sample_idx, gamma=0.1):
    ncut = Ncut(n_eig, gamma=gamma, track_grad=True)
    ncut.fit(features[sample_idx])
...
def _eig_for_added_node(context, set_A, set_B, new_nodes, n_eig : int = 50, gamma=0.1):
    ...
    eigvec, eigval = nystrom_ncut_fixed_sample_wrapper(full_input, n_eig, sample_idx, gamma=gamma)
...
def gradient_descent(context, set_A, set_B, new_nodes, n_eig,
    ...
    for i in range(max_iter):
        ...
        set_A_eigvec, set_B_eigvec, new_nodes_eigvec, eigval = _eig_for_added_node(context, set_A, set_B, new_nodes, n_eig=n_eig, gamma=gamma)
        ...
        # compute loss
        loss = F.smooth_l1_loss(new_nodes_eigvec, target_eigvec)
        loss.backward()
        optimizer.step()
        # for plotting
...
def interpolate_eigvec(context, set_A, set_B, n_eig : int = 50,
    ...
    if gamma is None:
        gamma = find_gamma_by_degree_after_fps(context, d_gamma=degree)
